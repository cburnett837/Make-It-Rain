...
enum CategoryAnalyticChartDisplayedMetric: String, CaseIterable, Identifiable {
    ...
    case income, expenses, budget, incomeMinusExpenses
            ...
            if let selectedMonth {
                VStack(spacing: 0) {
                    ...
                    HStack {
                        ...
                        case .income: selectedMonth.incomeString
                        case .expenses: selectedMonth.expensesString
                        case .budget: selectedMonth.budgetString
                        case .incomeMinusExpenses: String(selectedMonth.incomeMinusExpenses)
                        }
...
//                    .foregroundStyle(Color.fromName(colorTheme).opacity(0.5))
                    .zIndex(-5)
            }
                        
            if showAverage && config.enableShowAverage {
                RuleMark(y: .value("Average", data.map {
                        switch displayedMetric {
                        case .income: $0.income
                        case .expenses: $0.expenses
                        case .budget: $0.budget
...
//                    colors: [Color.fromName(colorTheme), .clear],
                    startPoint: .top,
                    endPoint: .bottom)
                )
            }
...
//            "Total: \((data.map { $0.expenses }.reduce(0.0, +).currencyWithDecimals(useWholeNumbers ? 0 : 2)))": Color.fromName(colorTheme),
