...
enum MapBottomPanelContent: Identifiable {
    var id: MapBottomPanelContent {return self}
...
struct SearchSuggestions: View {
    ...
    var body: some View {
        List {
            if mapModel.completions.isEmpty && !mapModel.recentQueries.isEmpty {
                recentsSection
            } else {
    ...
    var recentsSection: some View {
        Section("Recents") {
            ForEach(mapModel.recentQueries, id: \.self) { recent in
                Text(recent)
                    .onTapGesture {
...
struct StandardMapView: View {
    ...
    var body: some View {
        ...
        NavigationStack {
            ...
            .sheet(item: $mapModel.panelContent, onDismiss: {
                mapModel.selection = nil
                mapModel.panelContent = nil
            }) { content in
                Group {
                    if content == .details && mapModel.selectedMapItem != nil {
            ...
            .onMapCameraChange(frequency: .onEnd) {
                mapModel.visibleRegion = $0.region
            }
            /// Populate search suggestions as the user types in the search bar.
            .onChange(of: mapModel.searchQuery) {
                mapModel.getAutoCompletions(for: $1)
            }
            /// Handle focusing and unfocusing of the search bar.
            .onChange(of: focusedField) {
                handleFocusAndUnfocusOfSearchBar($0, $1)
            }
            ...
            .onChange(of: mapModel.selection) {
                mapModel.updateWithSelectedFeature($1, parentID: parentID, parentType: parentType)
            }
            ...
            .onChange(of: mapModel.selectedMapItem) {
                handleSelectedMapItem($0, $1)
            }
        }
    }
    ...
    func mapToolbar() -> some ToolbarContent {
        ...
        if !mapModel.searchQuery.isEmpty || !mapModel.searchResults.isEmpty {
            ToolbarItem(placement: .topBarTrailing) { adjustSearchbutton }
    ...
    var adjustSearchbutton: some View {
        Button {
            withAnimation {
                print("Searching with \(mapModel.lastSearchQuery)")
                mapModel.search(parentID: parentID, parentType: parentType, useLastQuery: true)
            }
        } label: {
    ...
    func handleFocusAndUnfocusOfSearchBar(_ oldValue: Int?, _ newValue: Int?) {
        /// Show search suggestion list when focusing on search bar.
        if newValue == 0 {
            mapModel.showSearchSuggestions = true
            /// Show the suggestions if you focus on the search bar when it already has text in it.
            if !mapModel.searchQuery.isEmpty {
                mapModel.getAutoCompletions(for: mapModel.searchQuery)
            }
        }
        
        /// Clear suggestions & search results when hitting search bar X.
        if newValue == nil {
            mapModel.showSearchSuggestions = false
            mapModel.searchResults.removeAll()
    ...
    func handleSelectedMapItem(_ oldValue: CBLocation?, _ newValue: CBLocation?) {
        if newValue == nil {
            mapModel.selection = nil
            
            /// If an item is selected, and we tap on the map to unselect it, show the search bar (aka reset the map).
            if mapModel.searchQuery.isEmpty && mapModel.panelContent == .details {
                withAnimation {
...
//    }
    ...
    var saveOrDeleteButton: some View {
        ...
        var isSaved: Bool { !locations.filter { $0.id == selection.id && $0.active }.isEmpty }
        return Button {
            withAnimation {
                if isSaved {
    ...
    var directionsMenu: some View {
        Menu {
            ...
            if mapModel.isCalculatingRoute {
                ProgressView()
                    .frame(maxWidth: .infinity)
                    .tint(.none)
            } else {
...
struct ClearMapSearchButton: View {
    //@Environment(\.dismissSearch) private var dismissSearch
    @Environment(MapModel.self) private var mapModel
...
//
//#if os(iOS)
//struct SearchSuggestionsOG: View {
//    @Environment(MapModel.self) private var mapModel
//    //@Environment(\.dismissSearch) var dismissSearch
//    var focusedField: FocusState<Int?>.Binding
//    var parentID: String
